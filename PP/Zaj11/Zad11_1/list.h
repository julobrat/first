...
Panstwa zadaniem jest napisanie listy dwukrotnie wi¹zanej z glowa, oraz funkcji do jej obslugi.
Poza tym prosze o zaimplementowanie struktury ListIterator, do poruszania sie po naszej liscie w obydwie strony.
Nasza Lista ma zawieraæ elementy typu T (szczegoly ponizej).
Nasza implementacja wzorowana C++owym std::list, aczkolwiek inna niz: http://www.cplusplus.com/reference/list/list/
...
typedef int_fast32_t T;
    ...
    extern "C"
    {
        ...
        List* constructList(type a){
            List *lista = malloc(sizeof(List));
            lista->first = malloc(sizeof(Node));
            lista->last = lista->first;
            lista->first->value = (type) a;
        ...
        {
            List *lista = malloc(sizeof(Node) * elements);
            for(int i = 0; i < elements; ++i)
            {
                lista->first->value = value;
            }
        }
        ...
        // funkcja usuwajaca element z List na danej pozycji, zwracaj¹c iterator do nastepnej pozycji, podobnie do: http://www.cplusplus.com/reference/list/list/erase/
